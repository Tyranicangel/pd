<?php
	set_time_limit(0);
		include('connect.php');
		$ddo=$_GET['ddo'];
		echo "<head><link rel='stylesheet' type='text/css' href='../front/styles/style_common.css'><title>LOC Rejected DDO-wise Report</title></head>";
    $result = pg_query("SELECT reqamount,userid,requestflag,hoa,requestuser,requestdate FROM locrequest WHERE requestflag='1' AND conf_flag='2' AND requestdate >= '2015-04-01' AND requestuser LIKE '$ddo%' ORDER BY requestuser ");
...
foreach ($arr as $key => $value) {
		...
		<td><a target='_blank' href='http://125.21.84.129/pd/reports/locrej_ddo_action.php?ddo=".$key."'>".$value['num']."</a></td>
