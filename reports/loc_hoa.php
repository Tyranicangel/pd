<?php
    ...
    $result = pg_query("SELECT reqamount,grantamount,userid,requestflag,hoa,requestuser,requestdate FROM locrequest WHERE requestflag='1' AND conf_flag='6' AND requestdate >= '2015-04-01' AND ddtime IS NOT NULL AND userid LIKE '$ddo%' ORDER BY userid ");
...
while($row = pg_fetch_array($result)) {
	...
	$hoa= $row['hoa'];
		if(isset($arr[$hoa]['reqamount'])) {
				...
				$arr[$hoa]['grantamount']= $arr[$hoa]['grantamount']+$row['grantamount'];
				$arr[$hoa]['num'] = $arr[$hoa]['num']+1;
		} else {
			$arr[$hoa]['userid']=$row['userid'];
				$arr[$hoa]['reqamount'] = $row['reqamount'];
				$arr[$hoa]['grantamount'] = $row['grantamount'];
				$arr[$hoa]['num'] = 1;				
		}
		$n++;
}
...
foreach ($arr as $key => $value) {
		...
		<td><a target='_blank' href='http://125.21.84.129/pd/reports/loc_hoa_action.php?hoa=".$key."&ddoc=".$value['userid']."'>".$value['num']."</a></td>
