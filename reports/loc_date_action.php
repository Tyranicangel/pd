...
$d=explode("-", $dt);
$date=$d[2]."-".$d[1]."-".$d[0];
echo "<head><link rel='stylesheet' type='text/css' href='../front/styles/style_common.css'><title>LOC Granted Date-wise Report</title></head>";
    $result = pg_query("SELECT reqamount,grantamount,userid,requestflag,hoa,requestdate,ddtime,requestuser,refno,requestdate FROM locrequest WHERE requestflag='1' AND conf_flag='6' AND requestdate >= '2015-04-01' AND requestuser LIKE '$ddo%' AND requestdate='$date' AND ddtime IS NOT NULL ORDER BY requestuser ");
