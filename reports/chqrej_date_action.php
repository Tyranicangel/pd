...
$d=explode("-", $dt);
$date=$d[2]."-".$d[1]."-".$d[0];
    echo "<head><link rel='stylesheet' type='text/css' href='../front/styles/style_common.css'><title>Cheques Rejected Date-wise Report</title></head>";
    $result = pg_query("SELECT chequeno,partyname,issueuser,hoa,partyamount,transdate FROM transactions WHERE transstatus='21' AND transtype='1' AND transdate >= '2015-04-01' AND issueuser LIKE '$ddo%' AND transdate='$date' ORDER BY issueuser ");
