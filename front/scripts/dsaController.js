app.controller("DsaController",function($scope,$http,$state,$rootScope,Logging,Commas){
...
app.controller("DsaMainController",function($scope,$http,$state,$rootScope,Logging,Commas){
	$scope.$emit("changeTitle",$state.current.views.content.data.title);
	$scope.showloader=true;
	$http({
		method:'GET',
		headers:{'X-CSRFToken':localStorage.token},
		url:$scope.requesturl+'/dsa_data'
	}).
	success(function(result){
		$scope.showloader=false;
		if(result[0]=='success')
		{
			if(result[1]=='nomap')
			{
				alert('Sorry your Account is not mapped to any HOA or chequebook user, please contact your STO/DTO/DD to map!');
				Logging.logout();
			}else
			{
...
app.controller("DsaRequestController",function($scope,$http,$state,$rootScope,Logging,Commas,Dates){
	$scope.Dates=Dates;
	$scope.$emit("changeTitle",$state.current.views.content.data.title);
	$scope.showloader=true;
	$http({
		method:'GET',
		headers:{'X-CSRFToken':localStorage.token},
		url:$scope.requesturl+'/dsa_requests'
	}).
	success(function(result){
		$scope.showloader=false;
		if(result[0]=='invalid')
...
app.controller("DsaLoclistController",function($scope,$http,$state,$rootScope,Logging,Commas,Dates){
	$scope.Dates=Dates;
	$scope.$emit("changeTitle",$state.current.views.content.data.title);
	$scope.showloader=true;
	$http({
		method:'GET',
		headers:{'X-CSRFToken':localStorage.token},
		url:$scope.requesturl+'/dsa_loclist'
	}).
	success(function(result){
		$scope.showloader=false;
		if(result[0]=='invalid')
...
app.controller("DsaChequeController",function($scope,$http,$state,$rootScope,Logging,Commas,$stateParams,Dates){
	$scope.Dates=Dates;
	$scope.$emit("changeTitle",$state.current.views.content.data.title);
	$scope.showloader=true;
	...
	$http({
		method:'GET',
		headers:{'X-CSRFToken':localStorage.token},
		url:$scope.requesturl+'/dsa_request_data',
	...
	$scope.get_book=function(){
		...
		if($scope.cheques.number)
		{
			$http({
				method:'GET',
				headers:{'X-CSRFToken':localStorage.token},
				url:$scope.requesturl+'/dsa_book_data',
	...
	$http({
		method:'GET',
		headers:{'X-CSRFToken':localStorage.token},
		url:$scope.requesturl+'/dsa_invent_data'
	}).
	success(function(result){
		$scope.showloader=false;
		if(result[0]=='invalid')
		...
		if($scope.approval=='reject')
		{
			...
			else
			{
				...
				$http({
					method:'POST',
					headers:{'X-CSRFToken':localStorage.token},
					url:$scope.requesturl+'/accept_request_dsa',
					data:{
			...
			else
			{
				...
				$http({
					...
					if(result[0]=='success')
					{
						if(result[1] == 'success')
						{
							alert('Cheque book request approved and forwarded to your treasury officer for approval!');
							$state.go('dsa.request');
						}
						else
						{
...
app.controller("DsaCreateController",function($scope,$http,$state,$rootScope,Logging,Commas,$stateParams){
	$scope.$emit("changeTitle",$state.current.views.content.data.title);
	$scope.account={};
	...
	$scope.$watch('account.actype',function(){
		if($scope.account.actype=='2')
		{
			$scope.account.lap=0;
		}
	});
	...
	$scope.checkddo = function() {

		if($scope.account.ddo) {

			var sto = $scope.usernamemain.substr(0, 4);

			if($scope.account.ddo.indexOf(sto) != 0) {

				alert("You cannot add this ddocode.");
				$scope.account.ddo = "";
			}
		}
	}
	...
	$scope.create_account=function(){
		...
		else
		{
			...
			$http({
				method:'POST',
				headers:{'X-CSRFToken':localStorage.token},
				url:$scope.requesturl+'/create_account_dsa',
				...
				if(result[0]=='success')
				{
					if(result[1]=='created')
					{
						alert('Account Created and forwarded to your Deputy Director for approval! User Ids '+$scope.account.ddo+'and '+$scope.account.ddo+'checker with password 123456 are created, the PD administrator can only login now into his/her account for now, all the functionalities will be activated only after your DD approves the account that you just created!');
					}
...
app.controller("DsaLocController",function($scope,$http,$state,$rootScope,Logging,Commas,$stateParams,Dates){
	$scope.Dates=Dates;
	$scope.$emit("changeTitle",$state.current.views.content.data.title);
	$scope.showloader=true;
	$scope.approval='approve';
	$http({
		method:'GET',
		headers:{'X-CSRFToken':localStorage.token},
		url:$scope.requesturl+'/dsa_loc_data',
	...
	$scope.req_accept=function(){
		...
		if($scope.approval=='reject')
		{
			...
			else
			{
				...
				$http({
					method:'POST',
					headers:{'X-CSRFToken':localStorage.token},
					url:$scope.requesturl+'/accept_loc_dsa',
					data:{
						user:$scope.requestdata.requestuser,
		...
		else
		{
			...
			else if(parseInt($scope.issue_amt)>(parseInt($scope.requestdata.accounts.balance)+parseInt($scope.requestdata.accounts.loc)) && $scope.requestdata.hoa !="8011001050001000000NVN")
			{
			...
			else
			{
				...
				$http({
					...
					if(result[0]=='success')
					{
						...
						{
							alert('LOC Approved and has been forwarded to your treasury Officer for approval!');
							$state.go('dsa.loclist');
						}
...
app.controller("DsaTransController",function($scope,$http,$state,$rootScope,Logging,Commas,Dates){
	$scope.Dates=Dates;
	...
	$http({
		method:'GET',
		headers:{'X-CSRFToken':localStorage.token},
		url:$scope.requesturl+'/dsa_trans'
	}).
	success(function(result){
		$scope.showloader=false;
		if(result[0]=='invalid')
...
app.controller("DsaConfirmController",function($scope,$http,$state,$rootScope,Logging,Commas,Dates,$stateParams){
	$scope.Dates=Dates;
	$scope.getcommas=function(dat){
		return Commas.getcomma(dat);
	}
	...
	$http({
		method:'GET',
		headers:{'X-CSRFToken':localStorage.token},
		url:$scope.requesturl+'/dsa_chq_data',
		...
		else
		{
			...
			angular.forEach($scope.dat.laprecinfo.laptrans,function(x){
				...
				x.transdate = Dates.getDate(x.transdate);
			});
			...
			else
			{
				...
				$scope.dat.laprecinfo.lapvalidate = Dates.getDate($scope.lapvaldate);
			}
...
setTimeout(function(){
	...
	{
		var bool; 
		if(!$scope.dat.acinfo.lapsableflag) {
	...
	$scope.chq_generate=function(){//get transactin id for cheques
		$scope.showloader=true;
		$http({
			method:'GET',
			url:'../front/get_transid.php',
			params:{issueuser:$scope.dat.issueuser, hoa:$scope.dat.hoa, chequeno:$scope.dat.chequeno, partyamount:$scope.dat.partyamount,transdate:$scope.dat.transdate}
		}).success(function(result){
			...
			if(result!='Fail')
			{
				$http({
					method:'GET',
					headers:{'X-CSRFToken':localStorage.token},
					url:$scope.requesturl+'/update_transid_dsa',
	...
	$scope.chq_confirm=function(){
		...
		if($scope.approval=='approve')
		{
			$http({
				...
				headers:{'X-CSRFToken':localStorage.token},
				url:$scope.requesturl+'/dsa_chqlist_confirm',
				data:{list:chqlist,rems:$scope.remarks, type:'approve'}
			}).
			success(function(result){
				$scope.showloader=false;
				if(result[0]=='success')
				...
				{
					alert('You are not mapped to any STO/ATO please contact your Deputy Director/DTO to map your login and then try again!');
					$state.go('dsa.trans');
				}
				else
				{
		...
		else
		{
			...
			if(!$scope.remarks) {
				...
				$http({
					...
					headers:{'X-CSRFToken':localStorage.token},
					url:$scope.requesturl+'/dsa_chqlist_confirm',
					data:{list:chqlist,rems:$scope.remarks, type:'reject'}
				}).
				success(function(result){
					$scope.showloader=false;
					if(result[0]=='success')
...
app.controller("DsaStatementController",function($scope,$http,$state,$rootScope,Logging,Commas,Dates){
	$scope.Dates=Dates;
	$scope.$emit("changeTitle",$state.current.views.content.data.title);
	$scope.showloader=true;
	...
	$http({
		method:'GET',
		headers:{'X-CSRFToken':localStorage.token},
		url:$scope.requesturl+'/get_dsa_admins'
	}).
	success(function(result){
		$scope.showloader=false;
		if(result[0]=='invalid')
	...
	$scope.admin_change=function(){
		...
		if($scope.pdamin!='select')
		{
			...
			$http({
				method:'GET',
				headers:{'X-CSRFToken':localStorage.token},
				url:$scope.requesturl+'/get_dsa_hoas',
